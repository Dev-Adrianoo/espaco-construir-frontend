    ...
    const fetchInitialData = async () => {
      ...
      if (!userId) {
        setLoggedGuardianError(
          "Você precisa estar logado para cadastrar um aluno."
        );
      ...
      try {
        ...
        if (user?.role === "PROFESSORA") {
          ...
          try {
            console.log(
              `[PASSO 1] - KLEITÃO DA MASSA Usuário é professora. Chamada para o Id: ${userId}`
            );

            const studentsResponse = await apiService.getStudentsRegisteredByMe(
              Number(userId)
            );

            console.log(
              `[PASSO 2 ] Resposta recebida da API: `,
              studentsResponse
            );
            ...
            if (Array.isArray(studentList)) {
              console.log(
                `[PASSO 3] - Sucesso! ${studentList.length} alunos encontrados. `
              );
              setChildren(studentList);
            } else {
              console.warn(
                "[AVISO] - a resposta para a Api não é um array ou está vazia...",
                studentList
              );
              setChildren([]);
            }
            ...
            setChildrenError(
              error.response?.data?.message ||
                "Erro ao carregar alunos da professora."
            );
          } finally {
          ...
          try {
            ...
            try {
              ...
              const childrenResponse =
                await studentService.getStudentsByResponsible(
                  String(guardianId)
                );
            ...
            const error = err as AxiosError<{ message: string }>;
            setLoggedGuardianError(
              "Erro ao carregar dados do responsável. Por favor, faça login novamente."
            );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.role === "RESPONSAVEL") {
        const updatedChildren = await studentService.getStudentsByResponsible(
          String(user.id)
        );
        setChildren(updatedChildren);
      } else if (user?.role === "PROFESSORA") {
        const updatedStudents = await apiService.getStudentsRegisteredByMe(
          Number(user.id)
        );
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/2 bg-white rounded-lg shadow p-6">
            ...
            <p className="text-gray-600 mb-6">
              Por favor, forneça as informações do aluno para personalizar a
      ...
      {isDeleteModalOpen && childToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">
              Tem certeza que deseja excluir o aluno {childToDelete.name}? Esta
